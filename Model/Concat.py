...
class Concat(ImageEncoderTextEncoderBase):
    """The TIRG model.
    ...
    def forward(self, x):
        if self.text_method == 'swem':
            x_f = torch.cat((self.extract_image_feature(x[0][0]), self.extract_text_feature(x[2][0])),1)
        else:
            x_f = torch.cat((self.extract_image_feature(x[0][0]), self.extract_text_feature(x[1][1])),1)
