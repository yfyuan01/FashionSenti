...
class AttributeOnlyModel(ImageEncoderTextEncoderBase):
    def __init__(self,args,**kwargs):
        super(AttributeOnlyModel,self).__init__(**kwargs)
        self.args = args
        ...
        self.classnum = 4262
        self.linear = nn.Linear(900, self.classnum)
    ...
    def forward(self, x):
        x_f = self.linear(self.extract_tag_attribute_feature(x[2][2]).mean(dim=1))
        x_t = x[2][1]
        return (x_f,x_t)
