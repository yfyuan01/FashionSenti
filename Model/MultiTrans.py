...
class MultimodalTransformer(nn.Module):
    def __init__(self, embed_dim=900, num_heads=2, attn_dropout=0.1, relu_dropout=0.1,
...
class MultitransLayer(nn.Module):
    def __init__(self,args,layers,num_heads=4,attn_dropout=0.0,relu_dropout=0.0,res_dropout=0.0,
                 embed_dropout=0.2,attn_mask=False,embed_dim=1024,**kwargs):
        super(MultitransLayer,self).__init__()
        ...
        for layer in range(layers):
            new_layer=MultimodalTransformer(embed_dim=self.embed_dim,
                                            num_heads=num_heads,
                                            attn_dropout=attn_dropout,
        ...
        self.layer_norm = NormalizationLayer(learn_scale=True,
...
class MultitransModel(ImageEncoderTextEncoderBase):
    def __init__(self, args, **kwargs):
        super(MultitransModel, self).__init__(**kwargs)
        self.args = args
        ...
        self.model['norm'] = NormalizationLayer(learn_scale=True,
                                                normalize_scale=normalize_scale)
        self.layers = 2
        self.model['multitrans'] = MultitransLayer(
            args = self.args,
            layers=self.layers
        )
        self.model['selfattn'] = MultitransLayer(
            args=self.args,
            layers = self.layers
        )
        self.orig_d_l = 900
        ...
        self.proj1 = nn.Linear(self.out_feature_image/2, self.out_feature_image/2)
    ...
    def forward(self, x):
        ...
        #x_img = self.extract_tag_image_feature(x[0][3])
        #print x_img.size()
        # x_img = self.extract_tag_image_feature(x[0][0])
        ...
        x_combine = self.model['multitrans'](x_txt,x_img,x_img)
        # x_combine = self.model['multitrans'](x_img,x_txt,x_txt)
        x_combine = self.model['selfattn'](x_combine)
        # x_combine1 = self.model['selfattn'](x_combine1)
        x_f = self.proj2(F.dropout(F.relu(self.proj1(x_combine[-1])),p=0.1,training=self.training))
        x_f+=x_combine[-1]
        # x_f1 = self.proj2(F.dropout(F.relu(self.proj1(x_combine1[-1])),p=0.1,training=self.training))
        # x_f1+=x_combine1[-1]
        # x_f+=x_img.squeeze(0)
        #x_combine = torch.cat((x_img,x_tag),dim=2)
        #x_f = self.model['linear'](x_combine.mean(dim=2))
